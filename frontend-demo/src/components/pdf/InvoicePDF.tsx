...
const styles = StyleSheet.create({
  ...
  storeName:  { fontSize: 18, fontFamily: 'Helvetica-Bold', color: '#111' },
  storeInfo:  { fontSize: 9, color: '#555', marginTop: 2 },
  invoiceTitle:{ fontSize: 22, fontFamily: 'Helvetica-Bold', color: '#2563eb', textAlign: 'right' },
  invoiceNum: { fontSize: 10, color: '#555', textAlign: 'right', marginTop: 2 },
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          <View>
            <Text style={styles.storeName}>{inv.store_name ?? 'Mahali Store'}</Text>
            {inv.store_address && <Text style={styles.storeInfo}>{inv.store_address}</Text>}
            {inv.store_phone && <Text style={styles.storeInfo}>Tel: {inv.store_phone}</Text>}
          </View>
          <View>
            <Text style={styles.invoiceTitle}>INVOICE</Text>
            <Text style={styles.invoiceNum}>{inv.invoice_number}</Text>
            <Text style={[styles.invoiceNum, { marginTop: 4 }]}>Date: {fmtDate(inv.created_at)}</Text>
          </View>
        </View>
