...
import { jobCardRepo } from '../database/repositories/workOrderRepo'
...
export function registerWorkOrderHandlers(): void {
  ...
  ipcMain.handle('jobCards:addAttachment', async (event, jobCardId: number, payload: unknown) => {
    try {
      const licErr = requireLicense('job_cards.edit')
      if (licErr) return err(licErr, 'ERR_LICENSE_REQUIRED')
      if (!authService.hasPermission(event.sender.id, 'repairs.edit')) return err('Forbidden', 'ERR_FORBIDDEN')
      ...
      await fsPromises.writeFile(fullPath, buf)
  ...
  ipcMain.handle('jobCards:deleteAttachment', (event, attachmentId: number) => {
    try {
      ...
      if (row.file_path && isAttachmentPathSafe(row.file_path)) {
        fsPromises.unlink(row.file_path).catch(() => { /* non-fatal */ })
      }
      return ok(true)
