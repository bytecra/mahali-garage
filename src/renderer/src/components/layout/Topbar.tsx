...
export default function Topbar({ collapsed, onToggle }: TopbarProps): JSX.Element {
  ...
  const { unreadCount, refresh } = useNotificationPoll(60_000)
    ...
    <header className="flex items-center justify-between h-14 px-4 border-b border-border bg-card shrink-0">
      ...
      <div className="flex items-center gap-2">
        ...
        <div ref={notifRef} className="relative">
          ...
          {/* Dropdown */}
          {notifOpen && (
            <div className="absolute end-0 top-full mt-2 w-80 bg-card border border-border rounded-xl shadow-2xl z-50 flex flex-col max-h-96">
              {/* Header */}
