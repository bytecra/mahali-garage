type ReportDepartment = 'Tech' | 'Both'
...
export function buildDailySalesPdf(params: {
  ...
  return layoutHtml(
    ...
    `
      ...
      <footer class="footer">
        Generated by Power Key — ${escapeHtml(generatedAt)}
      </footer>
    `,
  )
}
...
function fmtDeltaPct(prev: number, next: number): string {
  if (prev > 0.005) {
    const p = ((next - prev) / prev) * 100
    const arrow = p >= 0 ? '↑' : '↓'
    return `${arrow}${Math.abs(p).toFixed(1)}%`
  }
  if (next > 0.005) return '↑new'
  return '—'
}

export function buildDailySalesComparisonPdf(params: {
  storeName: string
  currency: string
  department: ReportDepartment
  period1Label: string
  ...
  rows1: DailySalesRow[]
  rows2: DailySalesRow[]
  dailyAligned: Array<{ day1: string; rev1: number; day2: string; rev2: number }>
}): string {
  const {
    storeName,
    currency,
  ...
  const generatedAt = nowLabel()
  const sum1 = rows1.reduce((s, r) => s + r.amount, 0)
  const sum2 = rows2.reduce((s, r) => s + r.amount, 0)
  const c1 = rows1.length
  const c2 = rows2.length

  const invRows = (rows: DailySalesRow[], cap: number): string =>
    rows
      .slice(0, cap)
      .map((row) => {
        const statusLower = row.status.toLowerCase()
  ...
  const dailyRows = dailyAligned
    .slice(0, 40)
    .map(
      (d) =>
        `<tr><td>${escapeHtml(d.day1)}</td><td class="amount">${escapeHtml(fmtAmount(d.rev1, currency))}</td><td>${escapeHtml(d.day2)}</td><td class="amount">${escapeHtml(fmtAmount(d.rev2, currency))}</td></tr>`,
    )
    .join('')

  const cap = 35
  const more1 = rows1.length > cap ? `<p style="font-size:11px;color:#64748b;">Showing first ${cap} of ${rows1.length} invoices (Period 1).</p>` : ''
  const more2 = rows2.length > cap ? `<p style="font-size:11px;color:#64748b;">Showing first ${cap} of ${rows2.length} invoices (Period 2).</p>` : ''
  ...
  return layoutHtml(
    ...
    `
      <header class="header">
        <h1 class="store-name">${escapeHtml(storeName)}</h1>
        <p class="subtitle">Daily Sales — Comparison Report</p>
      </header>
      <p style="margin:0 0 10px;font-size:12px;color:#475569;">
        <strong>Period 1 (${escapeHtml(period1Label)}):</strong> ${escapeHtml(period1From)} → ${escapeHtml(period1To)}
        &nbsp;|&nbsp;
        <strong>Period 2 (${escapeHtml(period2Label)}):</strong> ${escapeHtml(period2From)} → ${escapeHtml(period2To)}
      </p>
      <section class="meta">
        <div class="meta-item"><strong>Department:</strong> ${escapeHtml(department)}</div>
        <div class="meta-item"><strong>Generated At:</strong> ${escapeHtml(generatedAt)}</div>
      ...
      <section class="summary-grid summary-3">
        <div class="summary-box">
          <p class="summary-label">Total revenue (${escapeHtml(period1Label)})</p>
          <p class="summary-value">${escapeHtml(fmtAmount(sum1, currency))}</p>
        </div>
        <div class="summary-box">
          <p class="summary-label">Total revenue (${escapeHtml(period2Label)})</p>
          <p class="summary-value">${escapeHtml(fmtAmount(sum2, currency))}</p>
        </div>
        <div class="summary-box" style="border-color:#2563eb;">
          <p class="summary-label">Δ Revenue (${fmtDeltaPct(sum1, sum2)})</p>
          <p class="summary-value">${escapeHtml(fmtAmount(sum2 - sum1, currency))}</p>
        </div>
      </section>
      <section class="summary-grid summary-2">
        <div class="summary-box">
          <p class="summary-label">Transactions (${escapeHtml(period1Label)})</p>
          <p class="summary-value">${c1}</p>
        </div>
        <div class="summary-box">
          <p class="summary-label">Transactions (${escapeHtml(period2Label)})</p>
          <p class="summary-value">${c2}</p>
        </div>
      </section>

      <p style="margin:12px 0 6px;font-size:11px;font-weight:700;color:#1e3a8a;">Daily revenue (aligned by day index)</p>
      <section class="table-wrap">
        <table>
          <thead>
            <tr>
              <th>P1 date</th>
              <th class="amount">P1 revenue</th>
              <th>P2 date</th>
              <th class="amount">P2 revenue</th>
            </tr>
          </thead>
      ...
      <h3 style="font-size:13px;margin:16px 0 8px;">${escapeHtml(period1Label)} — invoices</h3>
      ${more1}
      <section class="table-wrap">
        <table>
          <thead>
          ...
          <tbody>${invRows(rows1, cap)}</tbody>
        </table>
      </section>

      <h3 style="font-size:13px;margin:16px 0 8px;">${escapeHtml(period2Label)} — invoices</h3>
      ${more2}
      <section class="table-wrap">
        <table>
          <thead>
...
export function buildProfitComparisonPdf(params: {
  storeName: string
  currency: string
  department: ReportDepartment
  period1Label: string
  ...
  period2From: string
  period2To: string
  total1: { revenue: number; cost: number; grossProfit: number }
  total2: { revenue: number; cost: number; grossProfit: number }
}): string {
  const {
    storeName,
    currency,
  ...
  const generatedAt = nowLabel()
  const m1a = total1.revenue > 0 ? (total1.grossProfit / total1.revenue) * 100 : 0
  const m2a = total2.revenue > 0 ? (total2.grossProfit / total2.revenue) * 100 : 0

  const row = (label: string, v1: number, v2: number, isMoney = true): string => {
    const a = isMoney ? fmtAmount(v1, currency) : `${v1.toFixed(2)}%`
    const b = isMoney ? fmtAmount(v2, currency) : `${v2.toFixed(2)}%`
    const d = fmtDeltaPct(v1, v2)
    return `<tr>
      <td>${escapeHtml(label)}</td>
      <td class="amount">${escapeHtml(a)}</td>
      <td class="amount">${escapeHtml(b)}</td>
      <td style="font-weight:700;">${escapeHtml(d)}</td>
    </tr>`
  }
  ...
  return layoutHtml(
    ...
    `
      <header class="header">
        <h1 class="store-name">${escapeHtml(storeName)}</h1>
        <p class="subtitle">Profit Report — Comparison</p>
      </header>
      <p style="margin:0 0 10px;font-size:12px;color:#475569;">
        <strong>Period 1:</strong> ${escapeHtml(period1From)} → ${escapeHtml(period1To)} (${escapeHtml(period1Label)})<br/>
        <strong>Period 2:</strong> ${escapeHtml(period2From)} → ${escapeHtml(period2To)} (${escapeHtml(period2Label)})
      </p>
      <section class="meta">
        <div class="meta-item"><strong>Department:</strong> ${escapeHtml(department)}</div>
        <div class="meta-item"><strong>Generated At:</strong> ${escapeHtml(generatedAt)}</div>
      ...
      <section class="table-wrap">
        <table>
          <thead>
            <tr>
              <th>Metric</th>
              <th class="amount">${escapeHtml(period1Label)}</th>
              <th class="amount">${escapeHtml(period2Label)}</th>
              <th>Δ %</th>
            </tr>
          </thead>
          <tbody>
            ${row('Revenue', total1.revenue, total2.revenue)}
            ${row('Cost (COGS)', total1.cost, total2.cost)}
            ${row('Gross profit', total1.grossProfit, total2.grossProfit)}
            ${row('Margin %', m1a, m2a, false)}
          </tbody>
        </table>
      </section>

      <section class="summary-grid summary-2" style="margin-top:14px;">
        <div class="summary-box">
          <p class="summary-label">Revenue ${escapeHtml(period1Label)}</p>
          <p class="summary-value">${escapeHtml(fmtAmount(total1.revenue, currency))}</p>
        </div>
        <div class="summary-box">
          <p class="summary-label">Revenue ${escapeHtml(period2Label)}</p>
          <p class="summary-value">${escapeHtml(fmtAmount(total2.revenue, currency))}</p>
        </div>
      </section>
...
export type TopProductComparisonRow = {
  product_name: string
  qty1: number
  rev1: number
  qty2: number
...
export function buildTopProductsComparisonPdf(params: {
  storeName: string
  currency: string
  department: ReportDepartment
  period1Label: string
  ...
  const body = rows
    .map((r) => {
      ...
      return `<tr>
        ...
        <td class="amount">${r.qty1}</td>
        <td class="amount">${escapeHtml(fmtAmount(r.rev1, currency))}</td>
        <td class="amount">${r.qty2}</td>
        <td class="amount">${escapeHtml(fmtAmount(r.rev2, currency))}</td>
        <td class="amount">${escapeHtml(fmtAmount(r.deltaRev, currency))}</td>
  ...
  return layoutHtml(
    'Top Products — Comparison Report',
    `
      <header class="header">
        <h1 class="store-name">${escapeHtml(storeName)}</h1>
        <p class="subtitle">Top Products — Comparison Report</p>
      </header>
      <p style="margin:0 0 10px;font-size:12px;color:#475569;">
        <strong>Period 1:</strong> ${escapeHtml(period1From)} → ${escapeHtml(period1To)} (${escapeHtml(period1Label)})<br/>
        <strong>Period 2:</strong> ${escapeHtml(period2From)} → ${escapeHtml(period2To)} (${escapeHtml(period2Label)})
      ...
      <section class="meta">
        <div class="meta-item"><strong>Department (label):</strong> ${escapeHtml(department)}</div>
        <div class="meta-item"><strong>Generated At:</strong> ${escapeHtml(generatedAt)}</div>
      </section>
      ...
      <section class="table-wrap">
        <table>
          <thead>
            <tr>
              <th>Product</th>
              <th class="amount">Qty P1</th>
              <th class="amount">Rev P1</th>
              <th class="amount">Qty P2</th>
              <th class="amount">Rev P2</th>
              <th class="amount">Δ Rev</th>
              <th>Δ %</th>
            </tr>
          </thead>
          <tbody>${body}</tbody>
...
export function buildEmployeePerformancePdf(params: {
  ...
  const rows = employees
    ...
    <tr style="${i % 2 === 0 ? '' : 'background:#f8fafc'}">
      ...
      <td style="padding:8px 10px;
        text-align:center;font-size:11px;">
        ${(e.tech_jobs + e.both_jobs) > 0
          ? `<span style="color:#3b82f6;">${e.tech_jobs + e.both_jobs}</span>`
          : ''}
      </td>
    </tr>
  `,
...
export function buildGenericTablePdf(opts: {
  ...
  return layoutHtml(opts.reportTitle, `
      ...
      ${summary}
      <p class="footer">End of Report — Power Key</p>
  `)
}
...
export function buildDepartmentReportsPdf(opts: {
  ...
  return layoutHtml('Department Reports', `
      ...
      ${block('Tech', opts.mechanical ?? opts.programming ?? opts.both)}
      <p class="footer">End of Report — Power Key</p>
  `)
}
